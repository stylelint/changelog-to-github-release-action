...
// src/changelogToGithubRelease.ts
function extractChangeItems({ version: version2 }) {
  return (tree) => {
    let list3;
    visit2(tree, "heading", (node2, index2, parent) => {
      const [text4] = node2.children;
      if (text4?.type === "text" && text4.value === version2 && parent && index2 !== void 0) {
        const nextSibling = parent.children[index2 + 1];
    ...
    if (!list3) {
      throw new Error(`Not found version: ${version2}`);
    }
    visit2(list3, "link", (node2) => {
      const [text4] = node2.children;
      if (text4?.type !== "text")
        return;
      node2.type = "text";
      node2.value = text4.value;
    });
    tree.children.length = 0;
    tree.children.push(list3);
  };
}
async function changelogToGithubRelease(changelog, version2) {
  const file = await remark().use(extractChangeItems, { version: version2 }).process(changelog);
  return String(file);
}
...
async function main() {
  ...
  const changelog = await (0, import_promises.readFile)(changelogPath, "utf-8");
